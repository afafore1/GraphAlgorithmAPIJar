...
//    private int _dest = -1;
    private HashMap<Vertex, Vertex> _path; // this is simple path from vertex to vertex
    ...
    public void disJointshortestPath(int v, int e) {
        ...
        while (pathExist(_graph.GetVertices().get(v), _graph.GetVertices().get(e)) == true) {
            ...
            _path = (HashMap<Vertex, Vertex>) tempShortPath.clone();
            //Gui.graph();
...
//        }
        _path.clear();
        _path = (HashMap<Vertex, Vertex>) setShortestPath.clone();
    }

    public HashMap<Vertex, Vertex> GetPathMap()
    {
    	return _path;
    }
